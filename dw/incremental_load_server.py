...
def incremental_batch_load(fk_servicio_amount, batch_size):
    ...
    , coalesce(cau.nombre, '') as causa_de_atraso
    
    , ser.fk_causas_atrasos, 
    ser.comentario_causas_atrasos, 
    ser.causas_cantidad
    ...
    left join public.causas_atrasos as cau on ser.fk_causas_atrasos=cau.id
    ...
    #df_proformas = connectionDB_todf2(proformas)
    ...
    #cargar_proformas(df_proformas)





def incremental_batch_load_online(fk_servicio_amount, batch_size):
    # Lista de valores de fk_servicio
    lista_fk_servicio = list(set(get_fk_servicio_values(fk_servicio_amount)))
    ...
    /* comercial */
    , concat ( case w
              n TRIM(comer.usu_nombre) LIKE '% %' then left(TRIM(comer.usu_nombre), strpos(TRIM(comer.usu_nombre), ' ') - 1) else TRIM(comer.usu_nombre) end ,' '
    , case when TRIM(comer.usu_apellido) LIKE '% %' then left(TRIM(comer.usu_apellido), strpos(TRIM(comer.usu_apellido), ' ') - 1) else TRIM(comer.usu_apellido) end ) as comercial_nombre /* ejecutivo comercial */
    ,comer.usu_rut as rut_comercial
...
def online_load(fk_servicio):
    ...
    lista_fk_servicio = list(set(get_fk_servicio_values(10)))
    # Lista de valores de fk_servicio
    ...
    while True:
        try:
            ...
            if initial_index + batch_size >= max_lenght: #si el largo de la query supera el indice del slice, saca hasta el final 
                print("final")
                df2 = df.iloc[initial_index - (max_id_etapa+1):]
...
def massive_load():
    ...
    index_dimension = load_dimensions(df, 0)
    index_fact = load_fact_table(df, 0)
