...
def load_dimensions(df, initial_index, stop_index, initial_index2, tipo_carga='increental'):
    """
    Parameters
    ----------
    ...
    #print(stop_index, initial_index, initial_index2)
    ...
    for index, row in df.iterrows():
        ...
        try:
            ...
            # Convertir a objeto de fecha
            fecha_objeto = datetime.strptime('01-01-3000', "%d-%m-%Y")
        ...
        ETA_nave = row['eta_fecha'] if not pd.isnull(row['eta_fecha']) else datetime(3000, 1, 1)
        ...
        try:
            ...
            fecha_real_arribo = datetime(3000, 1, 1)
        ...
        try:
            ...
            fecha_real_salida = datetime(3000, 1, 1)
...
def load_fact_table(df, initial_index, stop_index, initial_index2, tipo_carga='incremental'):
...
def loader(df, initial_index, batch_size, tipo_carga):
    ...
    while index < lenght + initial_index:
        #print("funcionando", initial_index2)
        try:
            index_dimension = index
            print("index_dimension", index_dimension,
                  index_dimension + batch_size, initial_index2)
            index_dimension = load_dimensions(
                df, index_dimension, index_dimension + batch_size, initial_index2)
        except:
        ...
        try:
            ...
            index_fact = load_fact_table(
                df, index_fact, index_fact + batch_size, initial_index2)
        except:
            print("fallo hechos")
            time.sleep(5)
            pass
...
# Llama a la función y pasa tu DataFrame como argum
...
#print(df.head(100))
initial_index, batch_size = 0, 25000000

# print(index)
tipo_carga = 'incrmental'
#load_dimensions(df, 0, 10, 0)
#loader(df, initial_index, batch_size, tipo_carga)
index_dimension = load_dimensions(df, initial_index, 20000000, 0)
index_fact = load_fact_table(df, initial_index, 20000000, 0)
"""
"""